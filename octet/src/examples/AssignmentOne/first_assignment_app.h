...
namespace octet {
	class first_assignment_app : public octet::app {
	...
	public:
		...
		first_assignment_app(int argc, char **argv) : app(argc, argv), font(512, 256, "assets/big.fnt") {
		}
		...
		void app_init() {
			...
			std::ifstream level("C:\\Users\\Javo\\Documents\\GitHub\\octet\\octet\\src\\examples\\AssignmentOne\\level.csv");
			std::string value;
			...
			// We use the enemy texture
			/*GLuint enemy = resource_dict::get_texture_handle(GL_RGBA, "assets/assignment/enemy.gif");
			sprites[enemy_sprite].init(enemy, 0, -1.72, 0.505f, 0.70f);*/
